...
export default function LabDashboard() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-gray-950 flex flex-col items-center justify-center p-10">
       ...
       <p className="text-[10px] font-black uppercase text-gray-500 tracking-[0.5em] animate-pulse">Setting up Laboratory Workspace...</p>
    </div>
  );
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-10 mb-10 sm:mb-16 pb-12 border-b border-gray-900">
           <div>
              <div className="flex items-center gap-3 mb-4">
                 ...
                 <span className="text-[10px] font-black text-gray-500 uppercase tracking-[0.4em]">Diagnostic Stream</span>
              </div>
              <h1 className="text-4xl sm:text-5xl font-black text-white italic uppercase tracking-tighter">Laboratory Console</h1>
              <p className="text-gray-500 mt-2 text-xs sm:text-sm font-medium">Technician: <span className="text-gray-300 font-bold">{user.fullName}</span> · Clinical Unit: Advanced Diagnostics</p>
           </div>
            ...
            <div className="lg:col-span-12 xl:col-span-8 space-y-6">
              {queue.length === 0 ? (
                <div className="py-32 text-center bg-gray-900/10 border border-gray-900 rounded-[3rem] opacity-40">
                   ...
                   <p className="text-lg font-black text-white italic uppercase tracking-tighter">No Pending Requests</p>
                </div>
              ) : (
                  ...
                  {queue.map((req) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-6 pt-6 border-t border-gray-800/50">
                        <div>
                           <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest mb-1">Patient Identity</p>
                           <p className="text-sm font-bold text-gray-300 truncate tracking-tight">{req.caseId?.patient?.fullName || 'N/A'}</p>
                        </div>
                        <div>
                           <p className="text-[9px] font-black text-gray-600 uppercase tracking-widest mb-1">Requesting Doctor</p>
            ...
            <div className="lg:col-span-12 xl:col-span-4 sticky top-12">
               <div className={`card !p-10 border-2 transition-all duration-500 ${!selectedRequest ? 'opacity-20 grayscale pointer-events-none translate-y-10' : 'border-brand-500/30'}`}>
                  <div className="flex items-center gap-4 mb-10 pb-6 border-b border-gray-800">
                     ...
                     <div>
                        <h2 className="text-lg font-black text-white italic uppercase tracking-tighter">Report Findings</h2>
                  ...
                  <form onSubmit={handleUpload} className="space-y-8">
                    <div>
                      <label className="block text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-3">Diagnostic Remarks</label>
                      <textarea 
                        value={comment}
                    ...
                    >
                      {uploading ? 'Sending Results...' : 'Submit Results →'}
                    </button>
                    {selectedRequest && (
                       <button 
                         type="button" 
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-gray-900/50 text-gray-500 border-b border-gray-800">
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest">Record Code</th>
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest">Patient Identity</th>
                      <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest">Report Date</th>
                      <th className="px-8 py-5 text-right text-[10px] font-black uppercase tracking-widest">Lab Record</th>
                    </tr>
                  </thead>
